...
UltraViolet and Optical Telescope (UVOT) observing campaigns, specifically for 
the 2022 campaign on NGC 6814 as published in [my paper](https://academic.oup.com/mnras/article/527/3/5569/7425644),
though the workflow and methods can be applied to any *Swift* campaign. The 
`functions.R` file contains all of the code for the methods used throughout this
workbook, so you should go over them to actually understand what is happening at
each step.

# Setting up the environment

The first thing that we need to do is load the method functions as well as the 
ggplot2 package with a customized theme for better visualization than the standard
`R` plotting methods offer.
...
As this workbook goes through my `R` workflow, I'm not going to go over how to 
actually process *Swift* UVOT data. The data can be publicly accessed via the 
[HEASARC data archive](https://heasarc.gsfc.nasa.gov/docs/archive.html). Once 
you've downloaded it, you'll need to process it using the tasks included in 
[HEASoft](https://heasarc.gsfc.nasa.gov/docs/software/lheasoft/) with the latest
...
analysis process outlined below. I've placed my `uvotsource` output FITS files in
the `uvotsource_output/` directory. We'll start by loading all of the output files
and merging them into a single `data.frame` which will have columns for the 
observation ID number, start time, UVOT filter, and flux values in units of counts/
second (`RATE`), ergs/s/cm^2/Angstrom (`FLUX.AA`), and milliJanksys (`FLUX.HZ`).

```{r readUVOTFITS}
uvot.lc <- lc.uvot(file.path = "uvotsource_outputfiles")
```

With all of the relevant observation data merged, we'll filter it to just work on
...
can convert into days and split the merged data into individual UVOT filter light
curves. Note that bad detections in UVOT are flagged with a count rate of -999, 
so we need to get rid of those while we're at it. Moreover, there are some instances
where the source flux exhibits rapid and significant dips. These may be the 
so-called 'UVOT drop-outs' that are associated with malfunctioning pixels on the
UVOT detector, and are thus not due to intrinsic source variability. We'll need 
to clean those up, and to do that we'll follow the method in the appendix of 
[Edelson et al. (2015)](10.1088/0004-637X/806/1/129).
...
Let's have a look at the light curve for one of our filters. Since the AGN SED 
typically dominates at the shortest wavelengths, that's the one that is most often
used as the so-called 'reference band' in the literature. Therefore, we'll use 
the W2 light curve as our example light curve and reference band moving forward.
...
The W2 light curve is clearly variable on short and long time-scales. This is 
great, because without this variability we would be limited to working with only
the time-averaged data products, which aren't as interesting in my opinion. Of 
course, performing a joint variability and time-average analysis is the most
powerful, which is what I did in my paper. Here, however, we'll just work with 
the variability products.

# Fractional variability spectrum

The simplest quantification of the variability process is fractional variability
($F_{\mathrm{var}}$), which offers a straightforward way in which we can assess 
the degree of variability in a light curve. [Edelson et al. (2002)](10.1086/323779)
and [Vaughan et al. (2003)](10.1046/j.1365-2966.2003.07042.x) present formalisms
for computing $F_{\mathrm{var}}$, with the latter method producing error bars 
~0.5x that of the former. We'll just use the [Edelson et al. (2002)](10.1086/323779)
method for now, since that's what I used in my paper. We can compute $F_{\mathrm{var}}$
in each UVOT filter, and then plot the results as a function of filter centroid 
wavelength according to [Poole et al. (2008)](10.1111/j.1365-2966.2007.12563.x)
in order to create an $F_{\mathrm{var}}$ spectrum.
...
We can see in the plot above that $F_{\mathrm{var}}$ decreases with wavelength. 
This suggests some damping mechanism with increasing strength at longer wavelengths.
A variable accretion disc emission component alongside an invariant host-galaxy 
emission component can broadly reproduce such behaviour. While $F_{\mathrm{var}}$
is useful, it is limited in that it simply quantifies the degree of variability
in a given light curve. If we want to delve deeper into the physical mechanisms
driving the observed variability, we need more sophisticated tools and methods.

# Time-lag analysis

## Interpolated cross-correlation function (ICCF)

The geometric configuration of the AGN central engine is often simplified as a
compact X-ray corona that illuminates a flat accretion disc. With a variable X-
ray coronal flux, we would expect coherent variations to be 'reverberated' in the
accretion disc emission with some time-lag ($\tau$) proportional to several key 
AGN properties. The wavelength dependence of the time-lags is expected to follow
$\tau \propto \lambda^{4/3}$. With our multi-wavelength UVOT light curves, we can
compute the time-lag between pairs of wavebands and evaluate the degree to which
this source follows the 'expected' behaviour. Typically, one filter is selected 
as the reference band to which all other filters are compared. Often, the W2 band 
is selected as the reference band as it is least contaminated by emission components 
outside of the AGN central engine. In some cases, however, longer wavelength bands 
should be used, such as those sources that are extremely reddened due to dust 
either in the Milky Way or AGN host galaxy. In our case, we'll use the W2 filter
as our reference band. We'll use the interpolated cross-correlation function 
(ICCF; [Gaskell & Sparke 1986](10.1086/164238), [Gaskell & Peterson 1987](10.1086/191216), 
[White & Peterson 1994](10.1086/133456), [Edelson et al. 2019](10.3847/1538-4357/aaf3b4)) 
to evaluate the time-lags in each filter with respect to W2. Moreover, we'll limit 
the lag calculation to those time-scales <1/3 the light curve length, as signals 
with <3 cycles are ambiguously detected. We'll use a time sampling of $\Delta\tau = 0.1~\mathrm{days}$.
...
At this point, it's clear that, when compared to the W2 filter, all of the UVOT
...
real and not just noise? and 2) how do we measure the time-lag? In the literature, 
question 1 is seldom addressed, but it's actually quite an important point that 
will be directly related to our answer for question 2 later. The basis of our 
answer is to try to compute what the ICCF would look like if the variability was
just noise. AGN variability follows a 'red-noise' process, with increasing 
variability at longer wavelengths. Thus, we need to simulate a sample of *realistic* 
AGN red-noise light curves to compute the expected 'noise' level in the ICCF. 

## Structure function (SF)

To do this, we'll use the simple method of [Timmer & Konig (1995)](https://ui.adsabs.harvard.edu/abs/1995A%26A...300..707T/abstract)
for which an AGN light curve is simulated based on an input power spectral density
...
The fitting procedure that we'll be using is maximum likelihood estimation (MLE),
...
The simple fit looks good. Therefore, we can approximate the W2 PSD slope ($\beta$) 
using the SF slope as $\beta = 1 + \alpha \approx 2.2$. Now we'll simulate $N=1000$ 
realizations of such a red-noise process to use as inputes when computing as many 
*noisy* ICCFs. We'll take the 99th percentile at each time-scale, which will give 
us an idea of how what correlation strengths from our *real* ICCFs correspond to
statistically significant signals (at the 99% level anyways). Since this process 
...
that many!) using the packages `foreach`, `parallel`, and `doParallel`. We'll plot 
the W2 vs U ICCF since it shows the largest time-lag.
...
We can see that despite $\mathrm{ICCF}_{\mathrm{W2},\mathrm{U}}\left(\left|\tau\right|\lesssim20~\mathrm{days}\right) > 0$,
the simulation results show us that really only $\mathrm{ICCF}_{\mathrm{W2},\mathrm{U}}\left(\left|\tau\right|\lesssim2~\mathrm{days}\right)$
is *statistically inconsistent with noise*. 

## ICCF centroid evaluation

Now that we can identify the significant
time-lags, we can work on actually evaluating the *value* of the lag itself. The
most straightforward approach is to simply take the time-lag associated with the 
peak in the ICCF ($\tau_{\mathrm{peak}}$). In practice, however, it is better to 
evaluate the centroid time-lag weighted by the ICCF ($\tau_{\mathrm{cent}}$) . 
To do this, we will use the flux randomization random subset sampling (FR/RSS) 
method of [Peterson et al. (1998)](10.1086/316177). This method takes into account 
the different sampling of the variability process across the different wavebands.
...
Clearly, although the ICCF peaks at very nearly $\tau=0~\mathrm{days}$, the centroid
is *positive*, meaning that the U variations *lag* the W2 variations by $\tau\approx0.3~\mathrm{days}$.
...
they convey the centroid distributions as well. We'll also include the $\tau\propto\lambda^(4/3)$
expected trend line to see how the data match it.
...
While there is clearly some wavelength-dependence to the time-lags, they do not 
match the expected relationship well, especially in the B and V bands. This may
be the result of long-term trends in the UVOT light curves unrelated to continuum
reverberation, which can dominate the time-lags. Since we're interested in rapid
variability on short time-scales, let's see what, if anything, changes if we de-
trend the long-term variations from the light curves.
...
of a user-specified order to a moving window with a user-specified width defined
as a number of consecutive data points. But how do we choose the values? Let's 
start simple, with a 1st order polynomial. Then, we just need to determine how 
many points wide our moving window should be. In the literature, the selection of
smoothing window width has been somewhat arbitrary. Here, we'll try to do it a 
bit more quantitatively by using the auto correlation function (ACF) of the W2 
filter. Let's see for what window width we can minimize the non-peak standard 
deviation, a choice that should result in light curves that are free of spurious 
correlations on long time-scales will also enhancing the precision of time-lag 
measurements.
...
From the plot above, it is clear that a width of 77 points minimizes the ACF non-
peak variance. Considering the median time separation between observations, this
corresponds approximately to a time smoothing of ~20.3 days. Let's have a look at
...
That looks pretty good. Let's see what's left over when we de-trend the light 
curve (original-trend).
...
The long-term trend is completely removed and what we're left with is the rapid 
variability that is most likely to be associated with reverberation off of the 
accretion disc, and thus the source of our lags. Let's see how the ACF has changed.
...
The original ACF peak is very broad, extending out to $\left|\tau\right|\approx20~\mathrm{days}$.
The new ACF peak is much narrower, only extending to $\left|\tau\right|\approx2.5~\mathrm{days}$. 
...
By design, the ICCFs now have significantly narrower peaks, which should enhance
the centroid estimation precision. Let's run the light curve simulations and FR/
RSS iterations to re-evaluate the lag-wavelength spectrum to compare with the 
previous results.

```{r detrendedlagwavelengthspectrum}
...
The de-trended lag-wavelength spectrum isn't too different from the previous one
(show in grey above), though errors are smaller and the trend is cleaner. In any 
case, the main result here is that the wavelength-dependence of the time-lags 
measured here still *do not* follow the predicted $\tau\propto\lambda^{4/3}$ 
trend! While the W2-U bands don't disagree significantly with the trend, the B 
and V bands are significantly under-predicted. In short, there is something non-
